...
// 真实 APK 的签名
#define APK_SIGN_MD5 "D7:DF:80:22:FC:C2:EC:67:54:14:1B:1C:5D:A9:23:AC"
...
Java_com_hao_happy_NativeUtil_getSign(JNIEnv *env, jclass clazz, jobject context) {
...
extern "C"
JNIEXPORT jstring JNICALL
Java_com_hao_happy_NativeUtil_checkSign(JNIEnv *env, jclass clazz, jobject context) {
    jbyteArray cert_byteArray = getSignatureByte(env, context);
    jsize size = env->GetArrayLength(cert_byteArray);
    jbyte* jbyteArray = new jbyte[size];
    env->GetByteArrayRegion(cert_byteArray, 0, size, jbyteArray);
...
extern "C"
JNIEXPORT jstring JNICALL
Java_com_hao_happy_NativeUtil_md5(JNIEnv *env, jclass clazz, jstring plaintext,jstring time) {
    const char * plaintextChar = env->GetStringUTFChars(plaintext, 0);
    const char * timeSt = env->GetStringUTFChars(time, 0);
    std::string plaintextStr = std::string(plaintextChar);
    char resultStr[1000] = {0};
    strcat(resultStr, plaintextChar);
    strcat(resultStr, timeSt);
    MD5 md5;
    std::string md5String = md5(resultStr);
    char * tabStr = new char [md5String.length()+1];
    strcpy(tabStr, md5String.c_str());
